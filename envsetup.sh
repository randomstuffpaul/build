...
- candyremote: Add git remote for CANDY Gerrit Review
- candygerrit: A Git wrapper that fetches/pushes patch from/to CANDY Gerrit Review
- candyrebase: Rebase a Gerrit change and push it again
- aospremote: Add git remote for matching AOSP repository
- cafremote: Add git remote for matching CodeAurora repository.
- mka:      Builds using SCHED_BATCH on all processors
- mkap:     Builds the module(s) using mka and pushes them to the device.
- candyka:     Cleans and builds using mka.
...
function candyremote()
{
    ...
    git remote rm candyremote 2> /dev/null
    GERRIT_REMOTE=$(git config --get remote.github.projectname)
    CMUSER=$(git config --get review.review.cyanogenmod.org.username)
    if [ -z "$CMUSER" ]
    then
        git remote add candyremote ssh://review.cyanogenmod.org:29418/$GERRIT_REMOTE
    else
        git remote add candyremote ssh://$CMUSER@review.cyanogenmod.org:29418/$GERRIT_REMOTE
    fi
    echo "Remote 'candyremote' created"
}
...
function makerecipe() {
  ...
  then
    pwd
    candyremote
    git push candyremote HEAD:refs/heads/'$1'
  fi
  '
}
